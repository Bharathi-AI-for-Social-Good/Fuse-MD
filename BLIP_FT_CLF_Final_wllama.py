...
# to load pre-trained BLIP model and Multilingual MuRIL model
# Hugging face transformers library used
from transformers import AutoProcessor, BlipForImageTextRetrieval, AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig
...
# Image encoder from BLIP pre-trained
...
# blip_epochs = 10
# blip_lrs = [1e-5, 2e-5, 3e-5, 4e-5, 5e-5]
...
blip = BLIP(vision_encoder, text_encoder).to(device).to(torch.bfloat16)
# blip.load_state_dict(torch.load("blip_LR" + str(blip_lr) + "_EPOCHS" + str(blip_epochs) + "_GAMMA" + str(blip_g) + "_BS" + str(N)))
...
for fusion in ["concat"]:  # , "element", "sumpool", "gated"
    ...
    for clf_lr in [1e-5, 2e-5, 3e-5, 4e-5, 5e-5]:
        ...
        blip encoders fixed in eval mode
        """
        for p in blip.parameters():
            p.requires_grad = False
            
        for p in list(blip.vision_embdg.parameters()) + list(blip.text_encoder.clf_head.parameters()):
            p.requires_grad = True
            
        for p in clf_head.parameters():
            p.requires_grad = True

        trainable_parameters = list(blip.vision_embdg.parameters()) + list(blip.text_encoder.clf_head.parameters()) + list(clf_head.parameters())
    ...
    #                 joint = torch.concat([v, t], dim = 1)
        ...
        with tqdm(test_loader, unit = "batch") as test_epoch:
            for img, lab, text, mask in test_epoch:
                ...
                pred = (out > best_thr).to(torch.bfloat16)
                correct += (pred == lab.unsqueeze(dim = 1)).sum()
        ...
        plt.savefig("predictions/"+str(language)+"/fusion/multimodal/"+str(language)+"_llama_blip_"+str(fusion)+"fusion_pretrained_"+str(clf_lr)+"_"+str(clf_epochs)+"_"+str(N)+".png")

        print(classification_report(tru, preds, digits = 5))

        cf = confusion_matrix(tru, preds)
        cr = classification_report(tru, preds, digits = 5)
        with open("predictions/"+str(language)+"/fusion/multimodal/"+str(language)+"_llama_blip_"+str(fusion)+"fusion_pretrained"+str(N)+".txt", "a") as text_file:
