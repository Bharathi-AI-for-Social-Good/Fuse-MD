...
model = AutoModelForCausalLM.from_pretrained("VishnuPJ/MalayaLLM_7B_Base", low_cpu_mem_usage=True, quantization_config=double_quant_config, torch_dtype=torch.float16)
...
"""
for clf_lr in [2e-5]:  #1e-5, , 3e-5,4e-5, 5e-5
    ...
    #save model
    torch.save(llama_clf.state_dict(), "saved_models/"+str(language)+"/malayalamllama/"+str(language)+"3_llama_pretrained_"+str(clf_lr)+"_"+str(epochs)+"_"+str(N)+".bin")
    
    #save confusion matrix
    plt.savefig("predictions/"+str(language)+"/text/"+str(language)+"3_llama_pretrained_"+str(clf_lr)+"_"+str(epochs)+"_"+str(N)+".png")

    print(classification_report(tru, preds, digits = 5))

    cf = confusion_matrix(tru, preds)
    cr = classification_report(tru, preds, digits = 5)
    with open("predictions/"+str(language)+"/text/"+str(language)+"3_llama_pretrained_"+str(N)+".txt", "a") as text_file:
